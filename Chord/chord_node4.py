    ...
    def process_lookup(self, pkg):
        if pkg.type == "LookUpRequest":
            ...
            if is_in_between(query_pos, self.predecessor_pos, self.pos_on_ring, self.keyspace_size):
                ...
                if query_key in self.keyvals:
                    content = "val={}".format(self.keyvals[query_key])
                    rep = LookUpReply(self.node_id, pkg.sender, pkg.id, content, pkg.initiator, client_req_id = pkg.client_req_id)
                    self.send(rep)
                    print("Client look-up request found!")
                else:
                    # print("but it's not because\ntable={}".format(self.keyvals))
                    self.in_queue.append(pkg)
            else:
                next_node_id = self.find_next_node_id(query_pos)
                req = LookUpRequest(self.node_id, next_node_id, pkg.content, pkg.initiator, client_req_id = pkg.client_req_id)
                self.req_to_fulfill[pkg.id] = pkg
                self.pkg_pointers[req.id] = pkg.id
                self.send(req)
        ...
        if pkg.type == "LookUpReply":
            ...
            if pkg.initiator == self.node_id:
                # print("LookUpReply made it back to local node! client_req_id={}".format(pkg.client_req_id))
                client = self.req_to_fulfill[pkg.client_req_id].client
                self.notify_client(client, pkg.content)
            else:
                rep = LookUpReply(self.node_id, prev_req.sender, prev_req.id, pkg.content, initiator=pkg.initiator,client_req_id = pkg.client_req_id)
                self.send(rep)
